(function() {
  ...
  var _generate_salt = function(seeded) {
    return _generate_iv(_settings.salt_length, seeded);
  };

  var seed = 1;
  var _seeded_random = function() {
      var x = Math.sin(seed++) * 10000;
      return x - Math.floor(x);
  }

  var _generate_iv = function (block_size, seeded) {
    if (seeded) {
      var buf = Buffer.alloc(block_size)
      for (var i = 0; i < block_size; i++)
        buf[i] = Math.floor(_seeded_random() * 255)
      return buf
    }
  ...
  RNCryptor.Encrypt = function(plain_text, password, version, seeded) {
    version || (version = 3);
    seeded || (seeded = false);
    ...
    components.headers.encryption_salt = _generate_salt(seeded);
    components.headers.hmac_salt = _generate_salt(seeded);
    components.headers.iv = _generate_iv(_settings.iv_length, seeded);
